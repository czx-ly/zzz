<template>
  <div class="add">
    ...
    <!-- 添加标签页 -->
    <el-form :model="addForm" lable-width="120px" ref="addForm">
      <el-tabs
        v-model="activeName"
      ...
      >
        ...
        <el-tab-pane label="商品参数" name="1">
          <el-form-item :label="first.attr_name" v-for="first in paramsDy" :key="first.attr_id">
            <!-- 如果在复选框租中绑定一个catecheckList, 那么在有多组的时候，会造成多组复选框都指向一个相同的数组 -->
...
//引入组件
import { qiullEditor } from "vue-quill-editor";
...
export default {
  ...
  methods: {
    //当做切换到 商品属性和商品参数，标签页的时候，判断用户如果没有选择分类，那么阻止标签页的跳转
      ...
      for (var i = 0; i < this.paramsDy.length; i++) {
        ...
        for (var j = 0; j < this.paramsDy[i].attr_vals.length; j++) {
          this.addForm.attrs.push({
            attr_id: id,
            attr_value: this.paramsDy[i].attr_vals[j]
          });
        }
      }
      //处理分类数据
    ...
    handleSuccess(response, file, fileList) {
      if (response.meta.status === 200) {
        //我们要的数据就在response, 我们要将上传成功之后的图片路径储存到addForm的pics中
